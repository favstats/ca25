...
secodndround <-   election_dat30 %>%
    ...
    mutate(value = str_remove_all(value, ", Canada")) %>% 
...
a(
  href = paste0("https://www.google.com/maps/search/?api=1&query=", URLencode(paste(value, "Canada"))),
...
region_dat <- election_dat30 %>%
  ...
  ungroup() %>%
  rename(region = value) %>%
  mutate(region = str_remove_all(region, ", Canada"))
